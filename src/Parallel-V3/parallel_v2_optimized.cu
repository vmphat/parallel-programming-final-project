...
float totalTimeAddBiasUnit = 0.0f;
float totalTimeapplySoftmax = 0.0f;
float totalTimeComputeCrossEntropyLoss = 0.0f;
float totalTimeOneHotEncoding = 0.0f;
float totalTimePredictLabels = 0.0f;
float totalTimeSubstractMatrix = 0.0f;
// =======================================================================
...
void addBiasUnit(float *input, int numCols, dim3 blockSize = dim3(1, 1))
{
    ...
    CHECK(cudaFree(d_in));
    MY_TIMER.Stop();
    totalTimeAddBiasUnit += MY_TIMER.Elapsed();
}
...
void applyRelu(float *input, int numRows, int numCols, dim3 blockSize = dim3(1, 1))
{
    ...
    // Call kernel function
    applyReluKernel<<<gridSize, threadPerBlock>>>(d_in, size);
    CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());      // Check for kernel launch errors
    // Free device memory
                  ...
                  dim3 blockSize = dim3(1, 1))
{
    ...
    dim3 gridSize((size - 1) / threadPerBlock + 1);
    // Call kernel function
    reluBackwardKernel<<<gridSize, threadPerBlock>>>(d_A, d_dLdA, d_dLdZ, size);
    CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());      // Check for kernel launch errors
    // Free device memory
...
void applySoftmax(float *input, int numRows, int numCols, dim3 blockSize = dim3(1, 1))
{
    ...
    CHECK(cudaFree(d_in));
    MY_TIMER.Stop();
    totalTimeapplySoftmax += MY_TIMER.Elapsed();
}
                      ...
                      dim3 blockSize = dim3(1, 1))
{
    ...
    dim3 gridSize((numCols * numRows - 1) / numElementPerBlock + 1);
    // Call kernel function
    subtractMatricesKernel<<<gridSize, numElementPerBlock>>>(d_A, d_B, d_C, numRows * numCols);
    //CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());      // Check for kernel launch errors
    // Free device memory
    ...
    CHECK(cudaFree(d_C));
    MY_TIMER.Stop();
    totalTimeSubstractMatrix += MY_TIMER.Elapsed();
}
                     ...
                     dim3 blockSize = dim3(1, 1))
{
    ...
    // Call kernel function
    transposeMatrixKernel<<<gridSize, blockSize>>>(d_in, numRows, numCols, d_out);
    //CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());      // Check for kernel launch errors
    // Free device memory
...
__global__ void oneHotEncodingKernel(int *labels, float *Y, int numClasses, int numSamples, int len, int stt)
{
    int sample = blockIdx.x * blockDim.x + threadIdx.x; 
    if (sample < len)
    {
        int label = labels[sample + stt * len];
        for (int row = 0; row < numClasses; ++row)
        {
            Y[row * numSamples + (sample + stt * len)] = (row == label) ? 1.0f : 0.0f;
        }
    }
}
                    ...
                    dim3 blockSize = dim3(1, 1))
{
    ...
    for (int  i = 0; i < 2; i++){
        ...
        len = numSamples / 3;
        CHECK(cudaMemcpyAsync(d_labels, labels, numSamples * sizeof(float), cudaMemcpyHostToDevice, streams[i]));
        //CHECK(cudaMemcpyAsync(d_Y, Y, numSamples * numClasses * sizeof(float), cudaMemcpyHostToDevice, streams[i]));
        oneHotEncodingKernel<<<numBlocksPerGrid, numThreadsPerBlock, 0, streams[i]>>>(d_labels, d_Y, numClasses, numSamples, len, i);
    }
    start = 2 * numSamples / 3;
    len = numSamples - start;
    CHECK(cudaMemcpyAsync(d_labels, labels, numSamples * sizeof(float), cudaMemcpyHostToDevice, streams[2]));
        //CHECK(cudaMemcpyAsync(d_Y, Y, numSamples * numClasses * sizeof(float), cudaMemcpyHostToDevice, streams[i]));
    oneHotEncodingKernel<<<numBlocksPerGrid, numThreadsPerBlock, 0, streams[2]>>>(d_labels, d_Y, numClasses, numSamples, len, 2);
    ...
    CHECK(cudaFree(d_Y));
    MY_TIMER.Stop();
    totalTimeOneHotEncoding += MY_TIMER.Elapsed();
}
...
__global__ void crossEntropyLossKernel(float *Y, float *P, float *loss,
                                       int numClasses, int numSamples, int len, int stt)
{
    int sample = blockIdx.x * blockDim.x + threadIdx.x;
    if (sample < len)
    {
        ...
        for (int row = 0; row < numClasses; ++row)
        {
            sum += Y[row * numSamples + (sample + stt * len)] * logf(P[row * numSamples + (sample + stt * len)] + FLT_MIN);
        }
        atomicAdd(loss, -sum);
    }
}
                             ...
                             dim3 blockSize = dim3(1, 1))
{
    ...
    *loss = 0.0f;
    ...
    for (int  i = 0; i < 2; i++){
        start = i * numSamples / 3;
        len = numSamples / 3;
        CHECK(cudaMemcpyAsync(d_P, P, numSamples * numClasses * sizeof(float), cudaMemcpyHostToDevice, streams[i]));
        CHECK(cudaMemcpyAsync(d_Y, Y, numSamples * numClasses * sizeof(float), cudaMemcpyHostToDevice, streams[i]));
        crossEntropyLossKernel<<<numBlocksPerGrid, numThreadsPerBlock, 0, streams[i]>>>(d_Y, d_P, d_loss, numClasses, numSamples, len, i);
    }

    start = 2 * numSamples / 3;
    len = numSamples - start;
    CHECK(cudaMemcpyAsync(d_P, P, numSamples * numClasses * sizeof(float), cudaMemcpyHostToDevice, streams[2]));
    CHECK(cudaMemcpyAsync(d_Y, Y, numSamples * numClasses * sizeof(float), cudaMemcpyHostToDevice, streams[2]));
    crossEntropyLossKernel<<<numBlocksPerGrid, numThreadsPerBlock, 0, streams[2]>>>(d_Y, d_P, d_loss, numClasses, numSamples, len, 2);
    ...
    *loss /= numSamples;
    MY_TIMER.Stop();
    totalTimeComputeCrossEntropyLoss += MY_TIMER.Elapsed();
}
                   ...
                   dim3 blockSize = dim3(1, 1))
{
    ...
    CHECK(cudaFree(d_P));
    MY_TIMER.Stop();
    totalTimePredictLabels += MY_TIMER.Elapsed();
}
...
void updateWeights(float learningRate, int batchSize, dim3 blockSize = dim3(1, 1))
{
    ...
    dim3 gridSize_W1((W1_ROWS * W1_COLS - 1) / numElementPerBlock + 1);     

    CHECK(cudaMemcpyAsync(d_mb_dLdW1, mb_dLdW1, W1_ROWS * W1_COLS * sizeof(float), cudaMemcpyHostToDevice, stream1));
    CHECK(cudaMemcpyAsync(d_W1, W1, W1_ROWS * W1_COLS * sizeof(float), cudaMemcpyHostToDevice, stream1));          
    updateWeightsKernel<<<gridSize_W1, numElementPerBlock, 0, stream1>>>(d_mb_dLdW1, d_W1, W1_ROWS * W1_COLS,
                                                    learningRate, batchSize);
    // ******** Update weights for layer 2 ********
    CHECK(cudaMemcpyAsync(d_mb_dLdW2, mb_dLdW2, W2_ROWS * W2_COLS * sizeof(float), cudaMemcpyHostToDevice, stream2));
    CHECK(cudaMemcpyAsync(d_W2, W2, W2_ROWS * W2_COLS * sizeof(float), cudaMemcpyHostToDevice, stream2));     
    dim3 gridSize_W2((W2_ROWS * W2_COLS - 1) / numElementPerBlock + 1);

    updateWeightsKernel<<<gridSize_W2, numElementPerBlock, 0 , stream2>>>(d_mb_dLdW2, d_W2, W2_ROWS * W2_COLS,
                                                    learningRate, batchSize);
    // ******** Update weights for layer 3 ********
    CHECK(cudaMemcpyAsync(d_mb_dLdW3, mb_dLdW3, W3_ROWS * W3_COLS * sizeof(float), cudaMemcpyHostToDevice, stream3));
    CHECK(cudaMemcpyAsync(d_W3, W3, W3_ROWS * W3_COLS * sizeof(float), cudaMemcpyHostToDevice, stream3));     
    dim3 gridSize_W3((W3_ROWS * W3_COLS - 1) / numElementPerBlock + 1);

    updateWeightsKernel<<<gridSize_W3, numElementPerBlock, 0 , stream3>>>(d_mb_dLdW3, d_W3, W3_ROWS * W3_COLS,
                                                    learningRate, batchSize);
...
int main(int argc, char *argv[])
{
    ...
    printf("(*) Average time to substract matrix in 1 epoch: %.3f ms\n", totalTimeSubstractMatrix / NUM_EPOCHS);
    printf("(*) Average time to apply softmax in 1 epoch: %.3f ms\n", totalTimeapplySoftmax / NUM_EPOCHS);
    printf("(*) Average time to add bias unit in 1 epoch: %.3f ms\n", totalTimeAddBiasUnit / NUM_EPOCHS);
    printf("(*) Average time to compute cross entropy loss in 1 epoch: %.3f ms\n", totalTimeComputeCrossEntropyLoss / NUM_EPOCHS);
    printf("(*) Average time to one hot encoding in 1 epoch: %.3f ms\n", totalTimeOneHotEncoding / NUM_EPOCHS);
