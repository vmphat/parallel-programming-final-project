...
__global__ void SubstractMatrixV1(float *in1, float *in2, float *out, int numRows, int numCols)
{
...
__global__ void SubstractMatrixV2(float *in1, float *in2, float *out, int size)
{
...
void SubstractMatrix(float * in1, float * in2, float * out, int length, int width, int verDevice, dim3 blockSize = dim3(1)){
    GpuTimer timer;
    timer.Start();
	if (verDevice == 0){
		...
		// Call kernel
		SubstractMatrixV1<<<gridSize, blockSize>>>(d_in1, d_in2, d_out, length, width);
		...
		// Call kernel
		SubstractMatrixV2<<<gridSize, numElementPerBlock>>>(d_in1, d_in2, d_out, length * width);
...
void checkSubstractMatrix(int argc, char ** argv){
    printf("***************** Substract Matrix *****************\n");
    ...
    SubstractMatrix(h_in1, h_in2, correct_out, m, n, 0);
	printf("\n");
    ...
    SubstractMatrix(h_in1, h_in2, h_out1, m, n, 1, blockSize);
	float err = checkCorrectFloat(h_out1, correct_out, m * n);
	printf("Error: %f\n\n", err);

    SubstractMatrix(h_in1, h_in2, h_out2, m, n, 2, blockSize);
...
{
	printDeviceInfo();
    checkSubstractMatrix(argc, argv);
    return 0;
}
