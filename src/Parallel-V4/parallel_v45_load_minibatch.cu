...
/** @brief Kernel function to copy data to mini-batch and one-hot encode labels */
...
/** @brief Launch the copy data to mini-batch and one-hot encode kernel */
void copyDataToMiniBatch(float *trainData, int *trainLabels, int *mb_Indices, float *mb_X1, float *mb_Y, int batchSize, dim3 blockSize)
{
    int numThreadsPerBlock = 2;
    int numBlocksPerGrid = (batchSize - 1) / numThreadsPerBlock + 1;

    // Launch kernel function
    copyDataToMiniBatchKernel<<<numBlocksPerGrid, numThreadsPerBlock>>>(trainData, trainLabels, mb_Indices, mb_X1, mb_Y, batchSize);
    ...
    for (int epoch = 0; epoch < NUM_EPOCHS; ++epoch)
    {
        ...
        for (int mb_StartIdx = 0; mb_StartIdx < NUM_TRAIN; mb_StartIdx += batchSize)
        {
            ...
            */
            ...
            copyDataToMiniBatch(trainData, trainLabels, mb_Indices, mb_X1, mb_Y, batchSize, blockSize);
