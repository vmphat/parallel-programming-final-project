...
// Stream for backpropagation when training model
cudaStream_t streamMainBackward; // Main flow of backpropagation
cudaStream_t streamBranchBackward[NUM_LAYERS]; // Branch flow of backpropagation
// Events for branches to wait for main flow to finish
cudaEvent_t eventBranchWaitMainBackward[NUM_LAYERS];
// =======================================================================
...
void CREATE_STREAMS()
{
    ...
    // Stream for backpropagation when training model
    CHECK(cudaStreamCreate(&streamMainBackward));
    for (int i = 0; i < NUM_LAYERS; ++i)
    {
        CHECK(cudaStreamCreate(&streamBranchBackward[i]));
    }
    // Events for branches to wait for main flow to finish
    for (int i = 0; i < NUM_LAYERS; ++i)
    {
        CHECK(cudaEventCreate(&eventBranchWaitMainBackward[i]));
    }    
}
...
void DESTROY_STREAMS()
{
    ...
    // Stream for backpropagation when training model
    CHECK(cudaStreamSynchronize(streamMainBackward));
    CHECK(cudaStreamDestroy(streamMainBackward));
    for (int i = 0; i < NUM_LAYERS; ++i)
    {
        // Sync before destroying stream
        CHECK(cudaStreamSynchronize(streamBranchBackward[i]));
        CHECK(cudaStreamDestroy(streamBranchBackward[i]));
    }
    // Events for branches to wait for main flow to finish
    for (int i = 0; i < NUM_LAYERS; ++i)
    {
        CHECK(cudaEventDestroy(eventBranchWaitMainBackward[i]));
    }
}
                      ...
                      dim3 blockSize)
{
    ...
    // Call kernel function on main backward stream
    subtractMatricesKernel<<<gridSize, numThreadsPerBlock, 0, streamMainBackward>>>(A, B, C, size);    
}
...
void backward(dim3 blockSize)
{
    ...
    // dLdZ3 = A3 - Y on main backward stream
    subtractMatrices(mb_A3, mb_Y, mb_dLdZ3, A3_ROWS, BATCH_SIZE, blockSize);
    CHECK(cudaGetLastError());

    // Branch W3 waits for the completion of main backward stream
    CHECK(cudaEventRecord(eventBranchWaitMainBackward[2], streamMainBackward)); // Send event to stream
    CHECK(cudaStreamWaitEvent(streamBranchBackward[2], eventBranchWaitMainBackward[2])); // Synchronize streams with each other
    
    // dLdW3 = dLdZ3 * X3T
    dim3 myBlockSize(TILE_WIDTH, TILE_WIDTH);
    dim3 gridSize_dLdW3((X3_ROWS - 1) / TILE_WIDTH + 1,
                        (W3_ROWS - 1) / TILE_WIDTH + 1);
    // Call kernel function on branch W3 - streamBranchBackward[2]
    matrixMultiplyWithTransposeKernel_ABT<<<gridSize_dLdW3, myBlockSize, 0, streamBranchBackward[2]>>>(
        mb_dLdZ3, mb_X3, mb_dLdW3, W3_ROWS, BATCH_SIZE, X3_ROWS);
    CHECK(cudaGetLastError());

    // Kernel fusion on main backward stream:
    // - dLdX3 = W3T * dLdZ3
    // - mb_dLdA2 = mb_dLdX3[1:,:]
    ...
    // Call kernel function on main backward stream
    matrixMultiplyWithTransposeReluBackwardKernel<<<gridSize_dLdX3, myBlockSize, 0, streamMainBackward>>>(
        W3, mb_dLdZ3, mb_dLdZ2, mb_A2, W3_COLS, W3_ROWS, BATCH_SIZE);
    CHECK(cudaGetLastError());
    ...
    // Branch W2 waits for the completion of main backward stream
    CHECK(cudaEventRecord(eventBranchWaitMainBackward[1], streamMainBackward)); // Send event to stream
    CHECK(cudaStreamWaitEvent(streamBranchBackward[1], eventBranchWaitMainBackward[1])); // Synchronize streams with each other

    // mb_dLdW2 = mb_dLdZ2 * mb_X2T
    dim3 gridSize_dLdW2((X2_ROWS - 1) / TILE_WIDTH + 1,
                        (W2_ROWS - 1) / TILE_WIDTH + 1);
    // Call kernel function on branch W2 - streamBranchBackward[1]
    matrixMultiplyWithTransposeKernel_ABT<<<gridSize_dLdW2, myBlockSize, 0, streamBranchBackward[1]>>>(
        mb_dLdZ2, mb_X2, mb_dLdW2, W2_ROWS, BATCH_SIZE, X2_ROWS);
    CHECK(cudaGetLastError());
    ...
    // Call kernel function on main backward stream
    matrixMultiplyWithTransposeReluBackwardKernel<<<gridSize_dLdX2, myBlockSize, 0, streamMainBackward>>>(
        W2, mb_dLdZ2, mb_dLdZ1, mb_A1, W2_COLS, W2_ROWS, BATCH_SIZE);
    CHECK(cudaGetLastError());
    ...
    // Branch W1 waits for the completion of main backward stream
    CHECK(cudaEventRecord(eventBranchWaitMainBackward[0], streamMainBackward)); // Send event to stream
    CHECK(cudaStreamWaitEvent(streamBranchBackward[0], eventBranchWaitMainBackward[0])); // Synchronize streams with each other

    // mb_dLdW1 = mb_dLdZ1 * mb_X1T
    dim3 gridSize_dLdW1((X1_ROWS - 1) / TILE_WIDTH + 1,
                        (W1_ROWS - 1) / TILE_WIDTH + 1);
    // Call kernel function on branch W1 - streamBranchBackward[0]
    matrixMultiplyWithTransposeKernel_ABT<<<gridSize_dLdW1, myBlockSize, 0, streamBranchBackward[0]>>>(
        mb_dLdZ1, mb_X1, mb_dLdW1, W1_ROWS, BATCH_SIZE, X1_ROWS);
    CHECK(cudaGetLastError());

    // Sync host with all branch streams
    for (int i = 0; i < NUM_LAYERS; ++i)
    {
        CHECK(cudaStreamSynchronize(streamBranchBackward[i]));
    }
