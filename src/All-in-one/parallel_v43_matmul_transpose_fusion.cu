...
void backward(dim3 blockSize)
{
    ...
    */
    // C = A * B, AT has size (n x m), B has size (n x k), C has size (m x k)
    dim3 gridSize_dLdX3((BATCH_SIZE - 1) / TILE_WIDTH + 1,
                        (W3_COLS - 1) / TILE_WIDTH + 1); // gridSize_dLdX3 : (W3_COLS, BATCH_SIZE)
    // Call kernel function: (AT, B, C, n, m, k)
    matrixMultiplyWithTransposeKernel_ATB<<<gridSize_dLdX3, myBlockSize>>>(W3, mb_dLdZ3, mb_dLdX3, W3_COLS, W3_ROWS, BATCH_SIZE);
    CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());
    ...
    */
    // C = AT * B, AT has size (n x m), B has size (n x k), C has size (m x k)
    dim3 gridSize_dLdX2((BATCH_SIZE - 1) / TILE_WIDTH + 1,
                        (W2_COLS - 1) / TILE_WIDTH + 1); // gridSize_dLdX2 : (W2_COLS, BATCH_SIZE)
    // Call kernel function: (AT, B, C, n, m, k)
    matrixMultiplyWithTransposeKernel_ATB<<<gridSize_dLdX2, myBlockSize>>>(W2, mb_dLdZ2, mb_dLdX2, W2_COLS, W2_ROWS, BATCH_SIZE);
    CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());
