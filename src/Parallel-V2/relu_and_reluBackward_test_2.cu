...
__global__ void applyReluKernelV2(float *input, int size)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size)
    {
        input[idx] = (input[idx] > 0.0f) ? input[idx] : 0.0f;
    }
}

__global__ void applyReluKernelV3(float *input, int size)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void applyRelu(float *input, int numRows, int numCols, dim3 blockSize = dim3(1, 1))
{
    ...
    // Call kernel function
    applyReluKernelV3<<<gridSize, threadPerBlock>>>(d_in, size);
    CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());      // Check for kernel launch errors
    // Free device memory
...
__global__ void reluBackwardKernelV2(const float *A, float *dLdA, float *dLdZ, int size)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size)
    {
        dLdZ[idx] = (A[idx] > 0.0f) ? dLdA[idx] : 0.0f;
    }
}

__global__ void reluBackwardKernelV3(const float *A, float *dLdA, float *dLdZ, int size)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
                  ...
                  dim3 blockSize = dim3(1, 1))
{
    ...
    dim3 gridSize((size - 1) / threadPerBlock + 1);
    // Call kernel function
    reluBackwardKernelV3<<<gridSize, threadPerBlock>>>(d_A, d_dLdA, d_dLdZ, size);
    CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());      // Check for kernel launch errors
    // Free device memory
