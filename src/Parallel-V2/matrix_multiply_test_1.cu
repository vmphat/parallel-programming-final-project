...
__global__ void matrixMultiplyKernelV1(float* A, float* B, float* C, int m, int n, int k)
{
                    ...
                    dim3 blockSize = dim3(1, 1))
{
    ...
    // Call kernel function
    matrixMultiplyKernelV1<<<gridSize, blockSize>>>(d_A, d_B, d_C, numRowsA, numColsA, numColsB);
    CHECK(cudaDeviceSynchronize()); // Synchronize to wait for kernel to finish
    CHECK(cudaGetLastError());      // Check for kernel launch errors
    // Free device memory
